...
import static com.homeserver.barnesbrothers.gemgame.handlers.B2DVars.SPEED;
...
public class Player extends B2DSprite {
    ...
    public void update(float dt) {
        ...
        if (this.getPosition().x >= (GemGame.V_WIDTH - (this.getWidth()/2) - (SSIZE*3))/PPM) {
            this.getBody().setLinearVelocity(-SPEED, 0);
        }
        ...
        if (this.getBody().getLinearVelocity().x < SPEED && this.getBody().getLinearVelocity().x > 0.0f) {
            this.getBody().setLinearVelocity(SPEED, this.getBody().getLinearVelocity().y);
        }
        if (this.getBody().getLinearVelocity().x > -SPEED && this.getBody().getLinearVelocity().x < 0.0f) {
            this.getBody().setLinearVelocity(-SPEED, this.getBody().getLinearVelocity().y);
        }
